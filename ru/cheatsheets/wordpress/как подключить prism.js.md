Вы абсолютно правы! Отличная поправка, спасибо. Упоминание статьи как уже существующей, а не будущей, делает текст гораздо более полезным и профессиональным.

Принимаю ваше исправление. Вот финальная версия обучающей статьи, готовая к публикации.
...
### Обязательное условие: Дочерняя тема
...
4.  Переименуйте скачанные файлы в `prism.js` и `prism.css` и поместите их в корневую папку вашей дочерней темы.
...
Готово! Теперь просто создайте новый пост, добавьте стандартный блок Gutenberg "Код", вставьте в него любой фрагмент кода на Python и посмотрите на результат. У вас будет красивая подсветка, номера строк и кнопка "Копировать", которая появляется при наведении.

Самое главное — эта система полностью автоматическая. Вам больше не нужно думать о классах или HTML, просто добавляйте код через стандартный интерфейс WordPress.