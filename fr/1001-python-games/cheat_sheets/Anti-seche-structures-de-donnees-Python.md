...
*   **Définition :** Les listes en Python sont des collections ordonnées et modifiables d'éléments. Cela signifie que vous pouvez ajouter, supprimer et modifier des éléments dans la liste, et l'ordre des éléments est important.
...
*   **Exemples :**
    ...
    # Suppression d'une paire par la méthode pop (avec retour de valeur)
    hobby = alice_dict.pop("hobby")
    print(f"Suppression avec retour de valeur : {alice_dict}, valeur : {hobby}")
...
*   **Exemples :**
    ...
    # boris_tuple[0] = "Boris" # Cela provoquera une erreur : TypeError: 'tuple' object does not support item assignment

    # Impossible d'ajouter un élément
    # boris_tuple.append(4) # Cela provoquera une erreur : AttributeError: 'tuple' object has no attribute 'append'

    # Impossible de supprimer un élément
    # del boris_tuple[0]  # Cela provoquera une erreur : TypeError: 'tuple' object doesn't support item deletion
    ```
...
*   **Représentation :** Pour créer un objet `SimpleNamespace`, il faut l'importer depuis `types` et lui passer des arguments nommés (ou ne pas en passer) :
     ```python
...
*  **Caractéristiques :**
    ...
    *  Les attributs sont accessibles via la notation par points, comme pour les objets ordinaires : `alice_namespace.name`
    *  Contrairement aux dictionnaires, l'ordre des attributs est conservé.
    *  Les champs peuvent être modifiés, mais de nouveaux champs ne peuvent pas être ajoutés directement.
...
*  **Exemples :**
    ...
    # Impossible d'ajouter un nouvel attribut directement
    # alice_namespace.occupation = "artiste" # Cela provoquera une erreur : AttributeError: 'SimpleNamespace' object has no attribute 'occupation'
