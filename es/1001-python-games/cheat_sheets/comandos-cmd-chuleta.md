...
*   **`assoc`**: Muestra o cambia las asociaciones de archivos.
    ...
    *   **Ejemplos:**
        *   `assoc .txt`: Mostrar tipo de archivo para la extensión `.txt`.
        *   `assoc .txt=txtfile`: Establecer tipo de archivo para la extensión `.txt` como `txtfile`.
*   **`call`**: Llama a un archivo por lotes desde otro.
    *   **Sintaxis:** `call [nombre_archivo_por_lotes]`
    *   **Ejemplo:** `call mibatch.bat`: Llamar al archivo por lotes `mibatch.bat`.
*   **`chcp`**: Cambia la página de códigos de la consola.
    *   **Sintaxis:** `chcp [número_página_de_códigos]`
    *   **Ejemplos:**
        *   `chcp 1251`: Establecer página de códigos para cirílico.
        *   `chcp`: Mostrar página de códigos actual.
*   **`choice`**: Permite al usuario elegir de una lista de opciones.
    ...
    *   **Ejemplo:** `choice /c sn /t 10 /d n "¿Realmente desea salir?"`
*   **`find`**: Busca una cadena de texto en un archivo.
    *   **Sintaxis:** `find "cadena" [nombre_archivo]`
    *   **Ejemplos:**
        *  `find "Hola" miarchivo.txt` - busca la cadena "Hola" en miarchivo.txt
...
*   **`findstr`**: Busca cadenas de texto en archivos (más potente que `find`).
    ...
    *   **Ejemplos:**
        *   `findstr /i "error" miarchivo.log`: Busca la cadena "error" (sin distinción entre mayúsculas y minúsculas) en `miarchivo.log`.
...
*   **`gpupdate`**: Actualiza las políticas de grupo.
    ...
    *   **Ejemplo:** `gpupdate /force` - forzar actualización de políticas de grupo
*   **`help` o `/?`**: Muestra la ayuda para un comando.
    *   **Sintaxis:** `help [nombre_comando]` o `[nombre_comando] /?`
    *   **Ejemplo:** `help dir` o `dir /?`: Mostrar ayuda para el comando `dir`.
...
*   **`prompt`**: Personaliza la cadena del símbolo del sistema.
    *   **Sintaxis:** `prompt [texto]`
    *   **Ejemplos:**
        *   `prompt $p$g`: Establecer la cadena del símbolo del sistema en la ruta actual.
        *   `prompt MiPrompt>`: Establecer la cadena del símbolo del sistema en `MiPrompt>`.
