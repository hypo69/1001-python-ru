...
*   **`chcp`**: Cambia la página de códigos de la consola.
    *   **Sintaxis:** `chcp [numero_pagina_codigos]`
    *   **Ejemplos:**
...
*   **`findstr`**: Busca cadenas de texto en archivos (análogo más potente de `find`).
...
*   **`prompt`**: Configura la cadena de solicitud de la línea de comandos.
    *   **Sintaxis:** `prompt [texto]`
    *   **Ejemplos:**
        *   `prompt $p$g`: Establecer la cadena de solicitud para la ruta actual.
        *   `prompt MyPrompt>`: Establecer la cadena de solicitud `MyPrompt>`.
